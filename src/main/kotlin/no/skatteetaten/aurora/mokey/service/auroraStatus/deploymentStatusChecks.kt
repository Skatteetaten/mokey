...
class PodNotReadyCheck(@Value("\${mokey.status.notready.duration:10m}") val notReadyDuration: Duration) : StatusCheck(
    ...
    override fun isFailing(app: DeployDetails, pods: List<PodDetails>, time: Instant): Boolean {
        fun notReadyOverThreshold(pod: OpenShiftPodExcerpt): Boolean {
            if (pod.startTime == null) return false

            try {
                val started = parse(pod.startTime)
                ...
                logger.warn(
                    "Kunne ikke parse startTime fra " +
                        "pod=${pod.name} " +
                        "value=${pod.startTime} " +
...
@Component
class AverageRestartErrorCheck(
    @Value("\${mokey.status.restart.error:100}") val averageRestartErrorThreshold: Int
) : StatusCheck(
    StatusDescription(
    ...
    override fun isFailing(
        ...
        time: Instant,
    ): Boolean = pods.isRestartsAboveThreshold(averageRestartErrorThreshold)
}

@Component
class AnyPodDownCheck : StatusCheck(
    StatusDescription(
...
class NoAvailablePodsCheck : StatusCheck(
    ...
    override fun isFailing(
        ...
        pods: List<PodDetails>,
        time: Instant
    ): Boolean = app.targetReplicas > app.availableReplicas && app.availableReplicas == 0
}

@Component
class DifferentDeploymentCheck(
    @Value("\${mokey.status.differentdeployment.hour:20}") val hourThreshold: Long
) : StatusCheck(
    StatusDescription(
    ...
    override fun isFailing(app: DeployDetails, pods: List<PodDetails>, time: Instant): Boolean {
        val threshold = time.minus(ofHours(hourThreshold))
        val numberOfDifferentDeployments = pods
            .map { it.openShiftPodExcerpt.replicaName }
            .distinct()
            .count()
        ...
        return when {
            ...
            numberOfDifferentDeployments == 1 -> false
                ...
                when {
                    p.openShiftPodExcerpt.startTime.isNullOrEmpty() -> {
...
@Component
class AverageRestartObserveCheck(
    @Value("\${mokey.status.restart.observe:20}") val averageRestartObserveThreshold: Int
) : StatusCheck(
    StatusDescription(
    ...
    override fun isFailing(
        ...
        pods: List<PodDetails>,
        time: Instant,
    ): Boolean = pods.isRestartsAboveThreshold(averageRestartObserveThreshold)
}

@Component
class AnyPodObserveCheck : StatusCheck(
    StatusDescription(
...
class DeployFailedCheck : StatusCheck(
    ...
    override fun isFailing(
        ...
        pods: List<PodDetails>,
        time: Instant,
    ): Boolean = app.lastDeployment == "failed"
}
...
class NoDeploymentCheck : StatusCheck(
    ...
    override fun isFailing(
        ...
        pods: List<PodDetails>,
        time: Instant,
    ): Boolean = app.lastDeployment == null
}
...
class OffCheck : StatusCheck(
    ...
    override fun isFailing(
        ...
        pods: List<PodDetails>,
        time: Instant,
    ): Boolean = app.targetReplicas == 0 && app.availableReplicas == 0 && app.scaledDown == null
}

@Component
class DeploymentInProgressCheck : StatusCheck(
    StatusDescription(
    ...
    override fun isFailing(
        ...
        pods: List<PodDetails>,
        time: Instant,
    ): Boolean = !finalPhases.contains(app.lastDeployment)
}
...
fun List<PodDetails>.isRestartsAboveThreshold(threshold: Int): Boolean {
    ...
    val totalRestarts = this.sumOf { it.openShiftPodExcerpt.containers.sumOf { c -> c.restartCount } }
    val averageRestarts = totalRestarts / this.size

    return averageRestarts > threshold
}

fun List<PodDetails>.validateStatus(validator: (status: String) -> Boolean): Boolean = this.any { pod ->
    pod.managementData.health?.deserialized?.let {
        val status = it.at("/status").textValue().uppercase(getDefault())
        validator(status)
    } ?: false
}
